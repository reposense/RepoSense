 ...
 */
public class SimilarityThresholdArgumentType implements ArgumentType<Double> {
    private static final String PARSE_EXCEPTION_MESSAGE_THRESHOLD_OUT_OF_BOUND =
            "Invalid threshold. It must be a number between 0 and 1.";

    public Double convert(ArgumentParser parser, Argument arg, String value) throws ArgumentParserException {
        double threshold = Double.parseDouble(value);

        if (Double.compare(threshold, 1.0) > 0 || Double.compare(threshold, 0) < 0) {
