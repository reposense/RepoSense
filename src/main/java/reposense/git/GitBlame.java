...
public class GitBlame {
    ...
    public static String blameLine(String root, String commitHash, String fileDirectory, int lineNumber) {
        ...
        String blameCommand = String.format(
                "git blame -w --line-porcelain %s -L %d,+1 -- %s", commitHash, lineNumber, fileDirectory);

        return StringsUtil.filterText(runCommand(rootPath, blameCommand), COMBINATION_REGEX + "|" + COMMIT_TIME_REGEX);
    }
}
