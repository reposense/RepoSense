...
1. Ubuntu JDK 8 (`mainbuild`): Runs both unit tests and system tests on JDK 1.8 running on supported Ubuntu versions. This job also produces a RepoSense report and the MarkBind documentation website for previewing.
2. macOS JDK 8 (`mainbuild`): Runs both unit tests and system tests on JDK 1.8 running on supported macOS versions.
3. Windows JDK 8 (`mainbuild`): Runs both unit tests and system tests on JDK 1.8 running on supported Windows versions.
