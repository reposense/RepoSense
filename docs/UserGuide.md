...
* `Sort within groups by`: sorting criteria within each group
  ...
  * `Variance` : each sub-group will be sorted by the average of the squared differences from the average number of lines of code contributed per day within each individual. Detailed definition of variance is located [here](https://en.wikipedia.org/wiki/Variance).
