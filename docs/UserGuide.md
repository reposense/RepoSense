...
Example:`--view` or `-v`
* **`--timezone, -t ZONE_ID`**: Indicates the timezone which will be used for the generated report. The timezone should be recognized by Java [ZoneId](https://docs.oracle.com/javase/8/docs/api/java/time/ZoneId.html) class. Suggested Format: `UTC[±hh[mm]]`. Default: system's default timezone.<br>
