 ...
 *
 * The wizard runs on a separate Vite server (port 9002). We use absolute URLs
 * for visits and intercepts so that the global support.js beforeEach
 * (which visits localhost:9000) is unaffected by these tests.
 */
...
describe('Config Wizard — Full Happy-Path Flow', () => {
  beforeEach(() => {
    ...
    cy.intercept('POST', `${WIZARD_BASE}/api/preview`, { yaml: 'title: My Flow Report\n' }).as('preview');
